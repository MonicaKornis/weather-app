...
app.get('/redirect', function (req, res) {
  ...
  if(req.session.csrf_string === state) {
    request.post({ url: 'https://github.com/login/oauth/access_token?' + qs.stringify({
        client_id: process.env.client_id,
      ...
      (error, response, body) => {
        ...
        console.log(qs.parse(body).csrf_string);
        req.session.csrf_string = qs.parse(body).csrf_string;
